---
title: "Tables, reports and reproducibility"
author: "agriRank Development Team"
date: "2026-08-12"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Tables, reports and reproducibility}
...
A reproducible analysis should retain not only the final p-value but also the declared design, method, random seed, package versions, missingness summary, effects, and contrasts.



## `agri_table()`: three examples

```r
x <- simulate_agri("crd", seed = 131)
fit <- np_crd(yield ~ treatment, x)
agri_table(fit, "omnibus")
agri_table(fit, "effects")
agri_table(fit, "missing")
```

## `agri_report()`: three examples

```r
agri_report(fit, file = tempfile(fileext = ".md"), format = "md", language = "en")
agri_report(fit, file = tempfile(fileext = ".qmd"), format = "qmd", language = "en")
if (requireNamespace("rmarkdown", quietly = TRUE) && rmarkdown::pandoc_available())
  agri_report(fit, file = tempfile(fileext = ".md"), format = "html", language = "en")
```

## `agri_dashboard()`: three examples

```r
agri_dashboard(fit)
agri_dashboard(fit, file = tempfile("yield-dashboard-", fileext = ".qmd"))
rm <- simulate_agri("repeated_missing", seed = 132, missing_rate = .10)
dm <- agri_design(height ~ treatment * time, rm, design = "repeated", subject = subject, within = time)
fm <- agri_rank(dm, method = "incomplete_wild", B = 299, missing_assumption = "MCAR")
agri_dashboard(fm, file = tempfile("repeated-dashboard-", fileext = ".qmd"))
```

## `export_results()`: three examples

```r
f1 <- tempfile(fileext = ".rds"); export_results(fit, f1)
str(readRDS(f1), max.level = 1)
f2 <- tempfile(fileext = ".rds"); export_results(fm, f2)
```

The report language for this documentation set is English. The implementation still retains the package's optional language argument for backward compatibility.
