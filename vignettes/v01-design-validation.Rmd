---
title: "Declaring and validating agricultural experiments"
author: "agriRank Development Team"
date: "2026-08-12"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Declaring and validating agricultural experiments}
...
A defensible analysis begins with the experimental unit and the randomization. `agri_design()` stores these features explicitly so that downstream methods cannot silently reinterpret a block, subject, whole plot, or repeated occasion.



## `agri_design()`: three examples

```r
# 1. Completely randomized design
crd <- simulate_agri("crd", seed = 11)
d_crd <- agri_design(yield ~ treatment, crd, design = "crd")
d_crd

# 2. Factorial RCBD
rcbd <- simulate_agri("rcbd", seed = 12)
rcbd$dose <- factor(rep(c("D1", "D2"), length.out = nrow(rcbd)))
d_rcbd <- agri_design(yield ~ treatment * dose, rcbd, design = "rcbd", block = block)
d_rcbd

# 3. Repeated measures
rm <- simulate_agri("repeated", seed = 13)
d_rm <- agri_design(height ~ treatment * time, rm, design = "repeated",
                    subject = subject, within = time)
d_rm
```

## `validate_agri_design()`: three examples

```r
# 1. Valid design
validate_agri_design(d_crd, error = FALSE)

# 2. Empty factorial cell is diagnosed
bad <- subset(fac <- simulate_agri("factorial", seed = 14), !(A == "A2" & B == "B3"))
d_bad <- agri_design(yield ~ A * B, bad, design = "factorial")
validate_agri_design(d_bad, error = FALSE)

# 3. Duplicate subject-by-time cell is a fatal design error
rm2 <- rbind(rm, rm[1, ])
d_dup <- agri_design(height ~ treatment * time, rm2, design = "repeated",
                     subject = subject, within = time)
validate_agri_design(d_dup, error = FALSE)
```

## `design_summary()`: three examples

```r
design_summary(d_crd)
design_summary(d_rcbd)
design_summary(d_rm)
```

## Why validation precedes analysis

A missing response and a missing design variable are not equivalent. The former may be handled by a suitable incomplete-data method; the latter can destroy the randomization identity of the observation. Likewise, technical replicates must not be mistaken for independent experimental units. The package therefore reports empty cells, duplicated repeated cells, missing design variables, and other structural problems before inferential routing.
