---
title: "Graphics, Tables, Reports, and Reproducibility"
author: "agriRank Development Team"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Graphics, Tables, Reports, and Reproducibility}
...
**Owns:** turning a fitted object into a figure, a table, a report and a record
that a reviewer can check and a future analyst can rerun.

---

# 1. Why this vignette exists

An analysis that cannot be communicated has not finished, and one that cannot be
rerun has not been done.

Both failures are ordinary. A figure exported as a screen-resolution bitmap
cannot be repaired for a journal. A table of means beside a rank test is
internally inconsistent. A methods section written by hand drifts from the code
that produced the numbers. A results object that exists only in one R session
cannot be checked by anyone.

Every function in this vignette exists to close one of those gaps, and all of
them work from the **fitted object** rather than from numbers copied out of it.
That is the point: what is reported cannot drift from what was run.

> **Generate the figure, the table and the methods text from the object that
> produced the result, not from a transcription of it.**
...
1. produce the standard design and regression figures from a fitted object;
2. explain why the default figures show observed data rather than only means;
...
5. choose colour that survives a colour-blind reader and a monochrome printer;
6. produce a manuscript table with the uncertainty attached to every estimate;
7. state what belongs in a table and what does not;
8. generate a reproducible methods-and-results skeleton from the object;
9. export a fitted object for a reviewer or a later session;
10. assemble a reproducibility record that lets a colleague reproduce the
    numbers exactly.
...
|---|---|---|
| `agri_plot()` | the standard design figures | a design fit |
| `agri_np_plot()` | the standard regression figures | a regression fit |
| `agri_np_curves()` | several engines overlaid | data and a set of engines |
| `agri_np_forest()` | coefficient intervals | a regression fit |
| `agri_np_levels()` | the response at each factor level | a regression fit |
| `agri_interactive()`, `agri_np_interactive()` | an HTML widget | either |
| `agri_theme()` | the shared journal styling | any ggplot |
| `agri_save_figure()` | a file at a journal width | any ggplot |
| `agri_table()` | a manuscript table | any fit |
| `agri_format_ci()` | an estimate with its interval, as text | numbers |
| `agri_report()` | a methods-and-results skeleton | any fit |
| `agri_dashboard()` | an assembled Quarto view | any fit |
| `export_results()` | a stored object | any fit |
...
```{r fig-data, fig.cap = "Observed values by treatment. Every point is one plot."}
agri_plot(fit_crd, type = "data")
```

### 5.1.1 Why the default shows the data

A figure of means with error bars hides bimodality, a single extreme plot, and
unequal spread. Each of those changes what the test means, and none is visible
in a bar chart.

This is not a stylistic preference. A rank-based test asks about distributions,
and a figure that shows only a location summary has withheld the quantity the
test examined.

## 5.2 Interaction

```{r fig-interaction, fig.cap = "Interaction figure. The question is whether the lines are parallel."}
if (exists("fit_sp")) print(agri_plot(fit_sp, type = "interaction"))
```

## 5.3 Missingness

```{r fig-missing, fig.cap = "Where the gaps are. Concentration in one treatment or at one occasion is the pattern to look for."}
agri_plot(
...
A missingness figure belongs in the supplementary material of any longitudinal
paper. It answers, at a glance, the question a reviewer will otherwise ask.
...
```{r fig-fit, fig.cap = "Fitted curve over the observed points."}
agri_np_plot(fit_ss, type = "fit")
```

```{r fig-residuals, fig.cap = "Residuals against fitted values. Structure here means the curve is missing something."}
agri_np_plot(fit_ss, type = "residuals")
```

```{r fig-derivative, fig.cap = "The slope of the fitted curve. Where it crosses zero the response turns over."}
...
```{r fig-integer, fig.cap = "Steps and crosses, not a continuous line. There is nothing between two admissible decisions."}
...
The interactive layer is for exploration on your own screen. It does not belong
in a manuscript, and the static figures above are what `agri_save_figure()`
exports.
...
```{r theme, fig.cap = "The same figure with the package theme applied explicitly."}
...
```{r theme-extend, fig.cap = "The theme is a starting point, not a constraint."}
if (exists("p")) {
  print(p + agri_theme() +
          ggplot2::coord_flip() +
...
Every figure this package produces is a plain `ggplot` object. Nothing is locked,
and the theme can be replaced entirely.

---

# 8. Colour that survives the reader

```{r colour, fig.cap = "Grouped curves in the colour-blind-safe default palette."}
set.seed(1306)
...
```{r colour-grey, fig.cap = "The same figure in grey tones, safe for monochrome print."}
agri_np_plot(gfit, predictor = "dose", group = "cultivar", palette = "grey")
```

## 8.1 Why this is an argument and not a manual override

Roughly one man in twelve has some form of colour vision deficiency, and many
journals still print figures in black and white. A palette chosen by hand is lost
the next time the figure is rebuilt; a palette chosen by argument survives.

The default is the Okabe-Ito palette, which is distinguishable under the common
forms of colour vision deficiency.

## 8.2 Units on the axis

```{r units, fig.cap = "Axis labels carrying their units."}
...
A figure exported as a screen-resolution PNG cannot be repaired later. Export the
vector version at the same time, always.

## 9.3 Interpretation

Decide the width before designing the figure. A panel that reads well at 16 cm is
often illegible at 8 cm, and the fix is fewer elements, not smaller text.
...
`agri_format_ci()` exists so that the pair travels into a manuscript table
together. An estimate quoted without its uncertainty is not a result.
...
simplified in the prose, the number of permutations is remembered wrongly, the
engine named is the one the analyst meant to use.

`agri_report()` writes the skeleton **from the fitted object**, so the reported
design, engine, replication and seeds cannot disagree with what was run.

## 11.2 It is a skeleton, not a manuscript

The generated text records what was done. It does not, and should not, contain
the interpretation. Edit it, add the biology, and keep the generated facts.
...
The dashboard assembles the figures and tables of one analysis into a single
Quarto document, which is convenient for a supervisor, a co-author, or a
laboratory record.
...
A stored object lets a reviewer, a co-author or your future self re-derive every
number in the paper, including the ones that did not make it in. A table of
numbers does not.
...
| `sessionInfo()` | backend versions change results at the margin |
...
Silent row deletion, from a missing value or an unused factor level, is among the
most common sources of irreproducible results, and it is invisible in the printed
output.
...
**Why it is wrong.** It hides bimodality, extreme plots and unequal spread, all
of which change what the test means.

**What prevents it.** `agri_plot(type = "data")` shows observed values by
default. See section 5.1.1.
...
**Why it is wrong.** The analysis did not estimate a mean, and the mean can order
the treatments differently from the ranks.

**What prevents it.** `agri_table(what = "effects")` returns the quantities the
...
**What prevents it.** Reporting `agri_effects()` beside them.
...
**Why it is wrong.** The result cannot be enlarged, edited, or submitted, and the
figure has to be rebuilt at revision.

**What prevents it.** `agri_save_figure()` at a named journal width, in both a
raster and a vector format. See section 9.

---

# 17. Choosing colour by hand

**The mistake.** `scale_colour_manual()` with colours picked from the screen.

**Why it is wrong.** It is lost on rebuild, and it is rarely checked against
colour vision deficiency or monochrome print.

**What prevents it.** `palette = "color"` or `"grey"` as an argument. See section
8.1.
...
**Why it is wrong.** The prose drifts from the code, and the drift is invisible
to everyone including the author.

**What prevents it.** `agri_report()` generates the skeleton from the object. See
section 11.1.
...
**Why it is wrong.** No number can be re-derived, and every reviewer question
requires rerunning the analysis from scratch, if the script still works.

**What prevents it.** `export_results()` and the record of section 12.2.
...
**Why it is wrong.** A missing value silently removes plots, and the reported n
no longer matches the analysed n.
...
| a record a reviewer can rerun | `export_results()` plus section 12.2 |
...
7. the stratum, the multiplicity adjustment, the replicate count and the seed,
   in footnotes;
8. a methods paragraph generated from the fitted object;
9. the fitted object and `sessionInfo()` stored with the manuscript;
10. the supplied and analysed row counts, checked and equal.

# 24. A worked reproducibility statement

> All figures and tables were generated from the fitted objects using agriRank
> 0.14.0, so that the reported design, engine, replication and seeds cannot
> differ from those used in the analysis. Figures were exported with
> `agri_save_figure()` at one-column width, as 600 dpi TIFF for submission and as
> PDF for archiving, using the package's colour-blind-safe palette. The fitted
> objects, the analysed data, all random seeds and the output of `sessionInfo()`
> are deposited with the supplementary material and are sufficient to reproduce
