---
title: "Batch workflows and inferential sensitivity"
author: "agriRank Development Team"
date: "2026-08-12"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Batch workflows and inferential sensitivity}
...
Batch analysis is convenient in agronomy, but convenience must not become silent multiplicity or method shopping. `agriRank` therefore separates batch fitting from across-response adjustment and separates sensitivity analysis from p-value selection.



## `agri_batch()`: three examples

```r
x <- simulate_agri("crd", seed = 111)
x$biomass <- x$yield + rnorm(nrow(x))
x$chlorophyll <- 30 + 0.8 * x$yield + rnorm(nrow(x), 0, 2)
d <- agri_design(yield ~ treatment, x, design = "crd")
agri_batch(d, responses = c("yield", "biomass"))
agri_batch(d, responses = c("yield", "biomass", "chlorophyll"), adjust_across = "BH")
agri_batch(d, responses = c("yield", "biomass", "chlorophyll"), adjust_across = "holm")
```

## `agri_sensitivity()`: three examples

```r
xf <- simulate_agri("factorial", seed = 112)
df <- agri_design(yield ~ A * B, xf, design = "factorial")
agri_sensitivity(df)
fit <- agri_rank(df)
agri_sensitivity(fit, methods = c("primary", "ART"))
agri_sensitivity(df, methods = c("primary", "ART", "permuco"), seed = 113)
```

## Interpretation rule

Sensitivity analysis asks whether the substantive conclusion changes under alternative admissible models or inferential calibrations. It must never be used to retain the method with the smallest p-value.
