---
title: "Split-plot experiments and hierarchical randomization"
author: "agriRank Development Team"
date: "2026-08-12"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Split-plot experiments and hierarchical randomization}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r agrirank-setup, include=FALSE}
library(agriRank)
```

Split-plot experiments contain at least two randomization strata. Whole-plot effects and subplot effects therefore do not share the same experimental error structure.



## State of the art

Permutation and rank-based repeated/split-plot methods can preserve hierarchical dependence when the resampling scheme matches the randomization. A method that ignores the whole-plot stratum can substantially misrepresent uncertainty. `agriRank` therefore declares `block`, `whole_plot`, and `subplot` explicitly.

## `np_splitplot()`: three examples

```r
x <- simulate_agri("split_plot", seed = 61, n = 6)
if (requireNamespace("permuco", quietly = TRUE))
  np_splitplot(yield ~ irrigation * cultivar, x, block = block,
               whole_plot = irrigation, subplot = cultivar, method = "permuco")
if (requireNamespace("ARTool", quietly = TRUE))
  np_splitplot(yield ~ irrigation * cultivar, x, block = block,
               whole_plot = irrigation, subplot = cultivar, method = "ART")
if (requireNamespace("permuco", quietly = TRUE) || requireNamespace("ARTool", quietly = TRUE))
  np_splitplot(yield ~ irrigation * cultivar, x, block = block,
               whole_plot = irrigation, subplot = cultivar, method = "auto")
```

## `agri_rank()`: three views of the same design

```r
d <- agri_design(yield ~ irrigation * cultivar, x, design = "split_plot",
                 block = block, whole_plot = irrigation, subplot = cultivar)
if (requireNamespace("permuco", quietly = TRUE)) agri_rank(d, method = "permuco")
if (requireNamespace("ARTool", quietly = TRUE)) agri_rank(d, method = "ART")
if (requireNamespace("permuco", quietly = TRUE) || requireNamespace("ARTool", quietly = TRUE)) agri_rank(d)
```

## `agri_plot()`: three examples

```r
agri_plot(d, type = "data")
fit <- if (requireNamespace("permuco", quietly = TRUE)) agri_rank(d, method = "permuco") else NULL
if (!is.null(fit)) agri_plot(fit, type = "interaction")
if (!is.null(fit)) agri_plot(fit, type = "effects")
```


## Split-split plots: a third randomization stratum

A split-split experiment contains three nested treatment application levels. For a whole-plot factor $A$, subplot factor $B$, and sub-subplot factor $C$ within block $R$, the randomization hierarchy is

$$R \rightarrow R{:}A \rightarrow R{:}A{:}B \rightarrow R{:}A{:}B{:}C.$$

The first three terms identify block, whole-plot unit, and subplot unit strata; the final level contains the sub-subplot observations. `agriRank` therefore requires all three treatment-level declarations and refuses a split-split design without `subsubplot=`.

```r
ss <- simulate_agri("split_split", seed = 62, n = 5)
dss <- agri_design(
  yield ~ irrigation * cultivar * timing, ss, design = "split_split",
  block = block, whole_plot = irrigation, subplot = cultivar,
  subsubplot = timing
)
dss
if (requireNamespace("permuco", quietly = TRUE))
  np_splitsplit(yield ~ irrigation * cultivar * timing, ss,
                block, irrigation, cultivar, timing, method = "permuco", np = 199)
if (requireNamespace("ARTool", quietly = TRUE))
  np_splitsplit(yield ~ irrigation * cultivar * timing, ss,
                block, irrigation, cultivar, timing, method = "ART")
```

## Strip plots: two perpendicular treatment randomizations

In a strip plot, factors $A$ and $B$ are randomized independently as perpendicular strips within each block. Their primary error strata differ:

$$A: R{:}A, \qquad B: R{:}B,$$

whereas $A\times B$ is evaluated at the strip-intersection/residual level. Treating a strip plot as an ordinary two-way factorial discards this randomization information.

```r
st <- simulate_agri("strip_plot", seed = 63, n = 5)
dst <- agri_design(
  yield ~ irrigation * nitrogen, st, design = "strip_plot",
  block = block, strip_a = irrigation, strip_b = nitrogen
)
dst
if (requireNamespace("ARTool", quietly = TRUE))
  np_stripplot(yield ~ irrigation * nitrogen, st, block, irrigation, nitrogen, method = "ART")
if (requireNamespace("permuco", quietly = TRUE))
  np_stripplot(yield ~ irrigation * nitrogen, st, block, irrigation, nitrogen, method = "permuco", np = 199)
```

The ART adapter represents random intercepts for block, block-by-strip-A units, and block-by-strip-B units. The permuco adapter encodes the corresponding error strata. These adapters remain subject to the numerical validation protocol in `VALIDATION_PLAN.md`.

## Current boundary

The package does not silently coerce an incomplete or structurally ambiguous split-plot experiment into a simpler independent design. Missingness and imbalance must be evaluated relative to the actual randomization strata.
## Core methodological references

The documentation uses the bibliographic year of the version of record. Core references were cross-checked against the publisher/journal record and at least one independent bibliographic source; see `inst/references/REFERENCE_VERIFICATION.md` and `inst/references/agriRank-methods-verified.ris`.

- Pauly, M., Brunner, E., & Konietschke, F. (2015). Asymptotic permutation tests in general factorial designs. *Journal of the Royal Statistical Society: Series B*, 77(2), 461–473. DOI: 10.1111/rssb.12073.
- Brunner, E., Konietschke, F., Pauly, M., & Puri, M. L. (2017). Rank-based procedures in factorial designs: hypotheses about non-parametric treatment effects. *Journal of the Royal Statistical Society: Series B*, 79(5), 1463–1485. DOI: 10.1111/rssb.12222.
- Umlauft, M., Konietschke, F., & Pauly, M. (2017). Rank-based permutation approaches for non-parametric factorial designs. *British Journal of Mathematical and Statistical Psychology*, 70(3), 368–390. DOI: 10.1111/bmsp.12089.
- Friedrich, S., Konietschke, F., & Pauly, M. (2017). A wild bootstrap approach for nonparametric repeated measurements. *Computational Statistics & Data Analysis*, 113, 38–52. DOI: 10.1016/j.csda.2016.06.016.
- Umlauft, M., Placzek, M., Konietschke, F., & Pauly, M. (2019). Wild bootstrapping rank-based procedures: multiple testing in nonparametric factorial repeated measures designs. *Journal of Multivariate Analysis*, 171, 176–192. DOI: 10.1016/j.jmva.2018.12.005.
- Noguchi, K., Gel, Y. R., Brunner, E., & Konietschke, F. (2012). nparLD: An R software package for the nonparametric analysis of longitudinal data in factorial experiments. *Journal of Statistical Software*, 50(12), 1–23. DOI: 10.18637/jss.v050.i12.
- Friedrich, S., Konietschke, F., & Pauly, M. (2019). MANOVA.RM: Resampling-based analysis of multivariate data and repeated measures designs. *The R Journal*, 11(2), 380–400. DOI: 10.32614/RJ-2019-051.
- Frossard, J., & Renaud, O. (2021). Permutation tests for regression, ANOVA, and comparison of signals: the permuco package. *Journal of Statistical Software*, 99(15). DOI: 10.18637/jss.v099.i15.
- Hothorn, T., Hornik, K., van de Wiel, M. A., & Zeileis, A. (2008). Implementing a class of permutation tests: the coin package. *Journal of Statistical Software*, 28(8), 1–23. DOI: 10.18637/jss.v028.i08.
- Brunner, E., Konietschke, F., Bathke, A. C., & Pauly, M. (2021). Ranks and pseudo-ranks: surprising results of certain rank tests in unbalanced designs. *International Statistical Review*, 89(2), 349–366. DOI: 10.1111/insr.12418.
- Konietschke, F., & Brunner, E. (2023). rankFD: An R software package for nonparametric analysis of general factorial designs. *The R Journal*, 15(1), 142–158. DOI: 10.32614/RJ-2023-029.
- Amro, L., Konietschke, F., & Pauly, M. (2024). Incompletely observed nonparametric factorial designs with repeated measurements: a wild bootstrap approach. *Biometrical Journal*, 66(8), e70008. DOI: 10.1002/bimj.70008.
- Jan, S.-L., & Shieh, G. (2025). Power and sample size procedures for the Friedman test. *Sankhya B*, 87, 686–711. DOI: 10.1007/s13571-025-00362-2.
- Thiel, N. et al. (2026). Resampling NANCOVA for small samples. *Computational Statistics & Data Analysis*, 215, 108290. DOI: 10.1016/j.csda.2025.108290.
- Wobbrock, J. O., Findlater, L., Gergle, D., & Higgins, J. J. (2011). The aligned rank transform for nonparametric factorial analyses using only ANOVA procedures. *CHI 2011*, 143–146. DOI: 10.1145/1978942.1978963.
- Elkin, L. A., Kay, M., Higgins, J. J., & Wobbrock, J. O. (2021). An aligned rank transform procedure for multifactor contrast tests. *UIST 2021*, 754–768. DOI: 10.1145/3472749.3474784.
